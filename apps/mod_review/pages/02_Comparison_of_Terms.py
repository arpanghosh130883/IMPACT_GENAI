...
from common_app_files.functions import process_docx_file, process_pdf_file, check_file_exceptions
from mod_review.common.comparison_of_terms_constants import (
    MESSAGE_VAR,
    WELCOME_MESSAGE,
...
if MESSAGE_VAR not in st.session_state:
    st.session_state[MESSAGE_VAR] = [
        {
            "role": "system",
            "content": COMPARISON_OF_TERMS_PROMPT,
        }
    ]
...
def upload_a_file():
    ...
    # Check for file-level exceptions
    is_valid, error_message = check_file_exceptions(uploaded_files)
    ...
    if st.button("Upload", disabled=uploaded_files is None or len(uploaded_files) < 2):
...
def modify_system_prompt():
    # set_system_prompt(COMPARISON_OF_TERMS_PROMPT)   
